...
export default function Lobby({user,navigate}){
  ...
  useEffect(()=>{load()},[])
  async function load(){
    const{data}=await supabase.from('campaigns').select('*,dm:dm_id(username)').eq('is_active',true).order('created_at',{ascending:false})
    if(data)setCampaigns(data)
    setLoading(false)
  }
  async function create(e){
    e.preventDefault()
    const{data,error}=await supabase.from('campaigns').insert({name:form.name,password_hash:form.password,description:form.description,dm_id:user.id,max_players:form.max_players}).select().single()
    if(error){alert('Erro: '+error.message);return}
    navigate('campaign',{campaignId:data.id})
  }
  async function join(cid){
    ...
    if(camp?.password_hash!==pass){alert('Senha incorreta!');return}
    const{data:mc}=await supabase.from('characters').select('*').eq('campaign_id',cid).eq('user_id',user.id).single()
  ...
  const input={padding:12,background:'#1a0c03',border:'2px solid #4a2508',borderRadius:5,color:'#fdf8f0',width:'100%'}
  ...
  return React.createElement('div',{style:page},
    ...
    React.createElement('main',{style:main},
      React.createElement('div',{style:{display:'grid',gridTemplateColumns:'1fr 1fr',gap:20,marginBottom:30}},
        React.createElement('button',{onClick:()=>{setShowCreate(true);setShowJoin(false)},style:{...card,textAlign:'left',cursor:'pointer',background:showCreate?'#4a2508':'#2d1605',border:showCreate?'3px solid #d4891a':'2px solid #4a2508'}},
        ...
        React.createElement('button',{onClick:()=>{setShowJoin(true);setShowCreate(false)},style:{...card,textAlign:'left',cursor:'pointer',background:showJoin?'#4a2508':'#2d1605',border:showJoin?'3px solid #d4891a':'2px solid #4a2508'}},
      ...
      React.createElement('div',{style:{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(300px,1fr))',gap:15,marginTop:15}},
        campaigns.filter(c=>c.name.toLowerCase().includes(search.toLowerCase())).map(c=>
          React.createElement('div',{key:c.id,style:card},
            React.createElement('p',{style:{color:'#8b4f0f',fontSize:14,margin:0}},'Mestre: '+(c.dm?.username||'???')),
            React.createElement('h3',{style:{color:'#fdf8f0',fontFamily:'Georgia,serif',margin:'10px 0'}},c.name),
            React.createElement('button',{onClick:()=>join(c.id),style:{padding:10,background:'#4a2508',color:'#fdf8f0',border:'1px solid #8b4f0f',borderRadius:5,cursor:'pointer',width:'100%',marginTop:10}},'🚪 Entrar')
          )
        )
      )
    )
